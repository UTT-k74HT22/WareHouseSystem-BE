...
public interface InventoryRepository extends
    ...
    @Query(value = """
        ...
        AND (l.type IS NULL OR l.type = 'STORAGE')
        ...
        ORDER BY (CASE WHEN l.type = 'STORAGE' THEN 1 ELSE 2 END) ASC, 
                 (i.on_hand_quantity - COALESCE(i.quarantine_quantity, 0) - i.reserved_quantity) DESC
        LIMIT 1
        FOR UPDATE
    """, nativeQuery = true)
    ...
    @Query("""
        SELECT COALESCE(SUM(i.onHandQuantity), 0)
        FROM Inventory i
        WHERE i.locationId = :locationId
        """)
    java.math.BigDecimal sumTotalQuantityByLocationId(@Param("locationId") String locationId);
}
